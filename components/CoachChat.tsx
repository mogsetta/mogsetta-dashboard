...
export default function CoachChat({
  ...
  const resolvedLessonContext =
    intent === "lesson" && system && module
  ...
  useEffect(() => {
    if (!resolvedLessonContext) return;
    ...
    try {
      const res = await fetch(`/api/coach/${coachId}`, {
        ...
        body: JSON.stringify({
          ...
          context: resolvedLessonContext
            ? resolvedLessonContext
            : system
            ? { system, module: module ?? "general" }
            : null,
        }),
      });
      ...
      <header style={{ marginBottom: 24 }}>
        ...
        {resolvedLessonContext && (
          <p style={{ marginTop: 6, fontSize: 12, opacity: 0.5 }}>
            Context: {resolvedLessonContext.system} / {resolvedLessonContext.module}
            {resolvedLessonContext.lesson ? ` / ${resolvedLessonContext.lesson}` : ""}
          </p>
        )}
      </header>
